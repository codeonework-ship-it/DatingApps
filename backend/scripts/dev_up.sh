...
if [[ -f "$ROOT_DIR/config/.env.local" ]]; then
...
: "${LOCAL_DATABASE_URL:=postgresql://postgres:root%40123@localhost:5433/dating_app?sslmode=disable}"
: "${LOCAL_POSTGREST_URL:=http://127.0.0.1:54321}"
: "${LOCAL_POSTGREST_ANON_KEY:=local-dev-key}"
: "${LOCAL_POSTGREST_SERVICE_ROLE:=}"
: "${SUPABASE_DB_HOST:=}"
...
: "${GATEWAY_SKIP_POSTGRES_PROBE:=true}"
: "${SUPABASE_URL:=}"
: "${SUPABASE_ANON_KEY:=}"
...
mkdir -p "$ROOT_DIR/${MEDIA_UPLOADS_DIR#./}"

if [[ "${USE_LOCAL_DB}" == "true" ]]; then
  if [[ -z "${DATABASE_URL}" ]]; then
    DATABASE_URL="${LOCAL_DATABASE_URL}"
  fi
  if [[ -z "${SUPABASE_URL}" || "${SUPABASE_URL}" == *"example.supabase.co"* ]]; then
    SUPABASE_URL="${LOCAL_POSTGREST_URL}"
  fi
  if [[ -z "${SUPABASE_ANON_KEY}" && -z "${SUPABASE_SERVICE_ROLE}" ]]; then
    SUPABASE_ANON_KEY="${LOCAL_POSTGREST_ANON_KEY}"
    SUPABASE_SERVICE_ROLE="${LOCAL_POSTGREST_SERVICE_ROLE}"
  fi
fi
...
export ENVIRONMENT LOG_LEVEL API_GATEWAY_ADDR MOBILE_BFF_ADDR \
  ...
  USE_LOCAL_DB LOCAL_DATABASE_URL LOCAL_POSTGREST_URL LOCAL_POSTGREST_ANON_KEY LOCAL_POSTGREST_SERVICE_ROLE \
  GATEWAY_SKIP_POSTGRES_PROBE \
  ...
  DEFAULT_UNLOCK_POLICY_VARIANT \
  MEDIA_UPLOADS_DIR MEDIA_PUBLIC_BASE_URL FILE_STORAGE_BACKEND USE_AWS_S3_STORAGE \
  AWS_S3_REGION AWS_S3_BUCKET AWS_S3_ENDPOINT AWS_S3_ACCESS_KEY_ID AWS_S3_SECRET_ACCESS_KEY \
  AWS_S3_PUBLIC_BASE_URL AWS_S3_PROFILE_PHOTOS_PREFIX AWS_S3_FORCE_PATH_STYLE \
...
if [[ -z "${SUPABASE_URL}" ]]; then
  echo "ERROR: SUPABASE_URL is empty. Set SUPABASE_URL directly, or provide LOCAL_POSTGREST_URL when USE_LOCAL_DB=true, or provide SUPABASE_DB_HOST (db.<project-ref>.supabase.co)." >&2
  exit 1
fi
